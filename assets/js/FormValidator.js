class FormValidator {
  ...
  validateElement(element) {
    ...
    if (
      "text" === element.type &&
      element.min &&
      element.value.length < element.min
    ) {
      this.writeFormErrorMessage(
        element,
        "Le contenu du champs doit faire au minimum " +
          element.min +
          " caractères",
      );
    ...
    if (
      ...
      this.writeFormErrorMessage(
        element,
        "Le nombre doit être supérieur à " + element.dataset.min,
      );
